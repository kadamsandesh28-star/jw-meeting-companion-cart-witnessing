...
export default function TerritoryProfile() {
  ...
  async function renderPdfPageToPng(
    ...
    // Load PDF.js only when a PDF is actually being shared.
  ...
  async function createCombinedPdf(): Promise<Blob> {
    // Load pdf-lib only when the user actually requests
    // a territory PDF.
    const {
      PDFDocument,
      rgb,
    ...
    const leftDetails: Array<[string, string]> = [
      [
        "Type",
        currentTerritory.type || "None",
      ],
      [
        "Status",
        currentTerritory.status || "None",
      ],
      [
        "Service Group",
        currentTerritory.assignedServiceGroupId ||
          "None",
      ],
      [
        "Map Reference",
        currentTerritory.mapReference ||
          "None",
      ],
    ];
    ...
    const rightDetails: Array<[string, string]> = [
      [
        "Last Worked",
        currentTerritory.lastWorked ||
          "Not Recorded",
      ],
      [
        "Next Due",
        currentTerritory.nextDue ||
          "Not Scheduled",
      ],
    ];
    ...
    if (!currentTerritory.attachment) {
      ...
      if (
        ...
        // Preserve additional pages
        // from a multi-page map PDF.
